---
title: CV
icon: fas fa-file-lines
order: 2
---

## Education

- <img class="affiliation-inline-logo" src="/images/affiliations/tsinghua.png" alt="Tsinghua University logo"> M.Eng. in Big Data Technology and Engineering, Tsinghua University, **09/2026 - 06/2028**
- <img class="affiliation-inline-logo" src="/images/affiliations/zju.png" alt="Zhejiang University logo"> B.Eng. in Mechanical Engineering, Zhejiang University, **09/2022 - 06/2026**
- <img class="affiliation-inline-logo" src="/images/affiliations/uiuc.svg" alt="University of Illinois Urbana-Champaign logo"> B.S. in Mechanical Engineering, University of Illinois Urbana-Champaign, **09/2022 - 06/2026**
- High school diploma, Shanghai Jincai High School, **08/2019 - 07/2022**

## Publications

- **D. Xia\***, **Y. Shi\***, Y. Mu, et al., [*RoboProcessBench: Benchmarking Process-Aware Understanding in Vision-Language Robotic Manipulation*](https://arxiv.org/pdf/2606.13040), arXiv preprint arXiv:2606.13040, 2026. NeurIPS 2026 under review.
- **S. Annamraju\***, **H. Nisar\***, **D. Xia\***, S. A. Deka, A. Horowitz, N. Milkovic, and D. M. Stipanovic, [*Robotic Trail Maker Platform for Rehabilitation in Neurological Conditions: Clinical Use Cases*](https://arxiv.org/pdf/2504.19230), arXiv preprint arXiv:2504.19230, 2025. International Journal of Social Robotics under review.

## Academic and Industrial Experience

- <img class="affiliation-inline-logo shlab-inline-logo" src="/images/affiliations/shlab.png" alt="Shanghai AI Laboratory logo"> Research Intern @ Shanghai AI Laboratory, **03/2026 - 08/2026**
  - Process-aware VLM/VLA benchmarking for embodied manipulation
  - pi0/pi0.5 fine-tuning, openpi, ARX-AC 2 deployment, Fast-WAM progress modeling

- <img class="affiliation-inline-logo" src="/images/affiliations/im-motors.svg" alt="IM Motors logo"> Algorithm Engineer Intern @ IM Motors, **08/2025 - 10/2025**
  - Qwen3-VL-8B supervised fine-tuning with real-world driving data
  - SynDiff-AD data augmentation for long-tail driving scenarios

- <img class="affiliation-inline-logo" src="/images/affiliations/uiuc.svg" alt="University of Illinois Urbana-Champaign logo"> Research Assistant @ University of Illinois Urbana-Champaign, **06/2024 - 04/2025**
  - Robotic rehabilitation trajectory prediction and assistance-level planning
  - Conv-BiLSTM, imitation learning, stochastic MPC

- <img class="affiliation-inline-logo" src="/images/affiliations/zju.png" alt="Zhejiang University logo"> Research Assistant @ Zhejiang University, **09/2023 - 12/2023**
  - Dexterous hand design & simulation
  - ROS2 & Gazebo & SolidWorks

## Selected Projects

- **Process-Aware VLA/WAM for Embodied Manipulation**: benchmarked process-aware robotic manipulation understanding, fine-tuned pi0/pi0.5 policies with tele-operated data, and explored Fast-WAM progress modeling.
- **Dual-System Vision-Language Navigation on Quadruped Robots**: combined YOLO-World and GLM 5.1 with confidence-gated intervention on DeepRobotics Lite 3.
- **Robotic Rehabilitation Therapy Planning**: developed Conv-BiLSTM trajectory prediction, bi-level imitation learning, and data-driven stochastic MPC for assistance planning.
- **VLM Reasoning Expert for Autonomous Driving**: fine-tuned Qwen3-VL-8B on real-world driving data and explored SynDiff-AD augmentation for long-tail scenarios.

## Skills

- Languages: Python, C++, LaTeX
- English: **TOEFL** 107, **CET4** 680, **CET6** 604
- Miscellaneous: Isaac Lab, MuJoCo, PyTorch, Arduino, Fusion 360, SolidWorks

You can also download my full CV PDF: [CV_Eng.pdf](/files/CV_Eng.pdf)
